...
export const repliRouter = express.Router();

repliRouter.use(express.json());

repliRouter.get("/", async (_req: Request, res: Response) => {
  try {
    ...
    const users = await collections[3].find({}).toArray();
